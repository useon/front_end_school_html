...
class Stack {
    ...
    // 그냥 꺼내는거임. pop에 대한 값만 빠져나올 수 있게끔 만든 것.
    pop(index = this.arr.length - 1) {
        // index가 입력이 안되었을 때
        // index가 입력이 되었을 때
        if (index === this.arr.length - 1) { // pop의 입력값이 없을 때
            return this.arr.pop();
        }

        let result = this.arr.splice(index, 1); // index에 해당하는 위치에 한개를 delete하겠다는 것 그 값을 result로 넘긴다. 
